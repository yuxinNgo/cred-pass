...
export function IssuerScreen() {
  const { issue, now, busy } = useWallet();
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setIssued(await issue({ type: String(data.get("type") ?? ""), expiresOn: String(data.get("expiresOn") ?? ""), holderName: String(data.get("holderName") ?? "") }));
  ...
  return <>
    ...
    <div className="form-grid"><section className="panel"><h2>Credential information</h2><p className="caption">Issuer: CredPass Demo Issuer</p><form onSubmit={submit}>
      ...
      <button className="button primary" type="submit" disabled={busy}><Icon name="plus" size={17}/>{busy ? "Saving credential…" : "Issue demo credential"}</button>
    </form></section><aside className="issuer-preview" aria-live="polite">{issued ? <><div className="success-heading"><Icon name="check" size={20}/><h2>Credential added to wallet</h2></div><CredentialCard credential={issued} now={now}/><Link href={`/wallet/${issued.id}`} className="button full-width">View credential<Icon name="arrow" size={17}/></Link><p className="caption">Saved to your browser-scoped wallet. Refresh preserves it; losing the workspace cookie loses access.</p></> : <div className="issuer-explainer"><Icon name="id" size={36}/><h2>One credential.<br/>Only the facts you need.</h2><p>Choose a type and expiration date. Your demo credential is stored in your own workspace.</p><hr/><p>Metadata is encrypted on the server, which can decrypt it. Authenticated Midnight issuance and proof generation remain pending.</p></div>}</aside></div>
  </>;
}
