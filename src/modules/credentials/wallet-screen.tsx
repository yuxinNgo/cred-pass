...
export function WalletScreen({ overview = false }: { overview?: boolean }) {
  ...
  return <>
    ...
    <div className="wallet-tools"><p><Icon name="lock" size={14}/>Saved for this browser. Losing the workspace cookie loses access. Use fictional data only.</p><div><button disabled={busy} onClick={() => { void restore(); }}>Restore samples</button><button disabled={busy} onClick={() => { void clear(); }}>Clear demo wallet</button></div>{busy && <p role="status">Saving wallet changes…</p>}</div>
  </>;
}
