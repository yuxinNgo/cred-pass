...
export function WalletScreen({ overview = false }: { overview?: boolean }) {
  const { credentials, now, clear, restore, busy } = useWallet();
  ...
  return <>
    ...
    <div className="wallet-tools"><p><Icon name="lock" size={14}/>Saved for this browser. Losing the workspace cookie loses access. Use fictional data only.</p><div><button disabled={busy} onClick={() => { if (window.confirm("Replace every credential in this browser’s wallet with the three samples? This cannot be undone.")) void restore(); }}>Restore samples</button><button disabled={busy} onClick={() => { if (window.confirm("Permanently clear this browser’s demo wallet? Other browsers’ wallets are not affected.")) void clear(); }}>Clear demo wallet</button></div>{busy && <p role="status">Saving wallet changes…</p>}</div>
  </>;
}
