...
**Current progress: ~45% of the intended credential/ZK product (rough scope estimate, not a measured completion metric or production-readiness score).** The wallet includes Neon persistence and individual demo revocation; the separate public Compact prototype now has secret-based issuer authorization, irreversible revocation, and ledger-time expiry tests. It is still a demo, not production identity infrastructure. These local changes are not deployed by this update.
...
The development adapter is real tested TypeScript logic, called by a server route. It does **not** generate proofs or call Midnight. The separate public Compact registry enforces a sealed issuer-secret commitment, issuer-only irreversible revocation, and strict Unix-second ledger-kernel expiry. `corepack pnpm test:contract` compiles with compiler 0.26.0 / language 0.18.0 `--skip-zk`, executes generated circuits using pinned runtime 0.9.0, and checks expired transcript replay in the ledger VM. These are local runtime tests, **not generated or verified ZK proofs, authenticated network-time evidence, or live blockchain operations**. See [contracts/README.md](contracts/README.md) for authorization, clock uncertainty, and public-data boundaries.

Next: private credential commitments and holder-bound witnesses; real adapter/proof generation and verification; trusted real-world issuer identity, authenticated holder recovery and external verifier boundaries; abuse controls and storage cleanup before opening a public service. The UI remains a database-backed demo, not connected to these circuits. Full revocation architecture, composite proofs, identity networks, issuer governance, and advanced selective disclosure remain outside this pass.
